...
pub fn maybe_decode_lzfse_with_fallback<'a>(
    data: &'a [u8],
) -> Result<Cow<'a, [u8]>, AppleCompressionError> {
    if !is_apple_compression_stream(data) {
        return Ok(Cow::Borrowed(data));
    }

    decode_lzfse_with_fallback(data).map(Cow::Owned)
}
...
mod wasm_tests {
    use super::{AppleCompressionError, decode_lzfse_with_fallback};
    use wasm_bindgen_test::wasm_bindgen_test;

    #[wasm_bindgen_test]
    fn invalid_apple_stream_reports_native_fallback_unavailable() {
        let err = decode_lzfse_with_fallback(b"bvx2not-valid-lzfse")
            .expect_err("invalid stream should fail on pure wasm path");
        assert!(matches!(
            err,
            AppleCompressionError::NativeFallbackUnavailable(_)
        ));
    }
}
